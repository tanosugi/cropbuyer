...
export default function CropUpdateForm(props) {
  const {
    id: idProp,
    crop: cropModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = cropRecord
      ? { ...initialValues, ...cropRecord }
      : initialValues;
    setName(cleanValues.name);
    setImage_url(cleanValues.image_url);
    setDescription(cleanValues.description);
    setPolygonColor(cleanValues.polygonColor);
    setErrors({});
  };
  const [cropRecord, setCropRecord] = React.useState(cropModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Crop, idProp)
        : cropModelProp;
      setCropRecord(record);
    };
    queryData();
  }, [idProp, cropModelProp]);
  React.useEffect(resetStateValues, [cropRecord]);
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Crop.copyOf(cropRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CropUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cropModelProp)}
